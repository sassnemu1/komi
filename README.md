...
(Malupasic), Печора с высоты (k0k00rt), тундра (APL). Метаданные, alt-тексты и ссылки на
...
(hero — `priority`), исходники сжаты до 1600–2400 px. У каждого снимка есть
...
Компонент `Interlude` — полноэкранная фото-интерлюдия между главами (три на
странице: Горка, тундра, долина Манараги) с многослойным параллаксом на scrub:
...
Пропы: `depth` (0…1, амплитуда; у Горки 0.7 — кадр с людьми) и `focus="top"` —
